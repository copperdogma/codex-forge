...
def test_survival_gate_damage():
    ...
    issues = _scan_text("1", text, None, None)
...
def test_conditional_choice_branch():
    ...
    issues = _scan_text("2", text, None, None)
...
def test_dice_damage_no_branch():
    ...
    issues = _scan_text("5", text, None, None)
    assert "dice_damage_no_branch" in _codes(issues)


def test_terminal_outcome_text():
    text = "The beast crushes you. You die."
    issues = _scan_text("6", text, None, None)
    assert "terminal_outcome_text" in _codes(issues)


def test_multi_item_requirement():
    text = "If you have the key and the map, turn to 77."
    issues = _scan_text("7", text, None, None)
