...
## Success Criteria
- [ ] `validate_ff_engine_v2` (or wrapper) augments its report with traces for every missing/empty item: boundary module/evidence, start/end element ids, element text/page.
...
- [ ] Traces are included in `validation_report.json` and visible in pipeline logs.

## Tasks
- [ ] Enhance `validate_ff_engine_v2` to optionally load nearby artifacts (`section_boundaries_merged.jsonl`, `elements.jsonl`, `portions_enriched*.jsonl`) and attach per-section forensic traces for missing text/choices/sections.
...
- [ ] Include span metadata in traces: end_element_id, start/end seq/page, span length, and zero-length flag to spotlight empty spans.
- [ ] Record artifact provenance (mtime or hash) in traces to guard against stale inputs.
- [ ] Add portion-text snippet (if present) alongside start element text to ease eyeballing mismatches.
- [ ] Emit `suggested_action` per failing section (e.g., “rerun repair_portions on SID”, “rebuild boundary from ai_scan”, “classify ending”) to feed automated escalations.
